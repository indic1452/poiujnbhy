...
class ПодсказкаПроHttps(unittest.TestCase):
    """Сказать «нельзя» мало: человек уходит ни с чем, а выход в одну команду."""

    def test_названа_команда_включения(self):
        кусок = APP[APP.index("Этот браузер показывает такие уведомления"):][:900]
        self.assertIn("setup-https.ps1", кусок)

    def test_сказано_что_будет_после(self):
        кусок = APP[APP.index("Этот браузер показывает такие уведомления"):][:900]
        self.assertIn("переключатель", кусок)
