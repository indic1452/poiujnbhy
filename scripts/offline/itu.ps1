...
.DESCRIPTION
    ...
    КАК УСТРОЕНО. Ссылки не выдумываются из номера, а вычитываются со
    страниц самого МСЭ, а страниц этих три ступени:

        перечень серии    /rec/T-REC-A/en
        список изданий    /rec/recommendation.asp?parent=T-REC-A.31
        страница издания  /rec/recommendation.asp?parent=T-REC-A.31-200810-I

    Вторую ступень легко принять за страницу документа, но файла на ней нет:
    параметр назван «parent» не случайно, это перечень редакций. Берётся самая
    свежая — она и есть действующая. Если и на странице издания ссылки не
    оказалось (так бывает), адрес файла собирается из ключа издания, взятого
    оттуда же, и скачанное проверяется на подпись формата.

    Адреса разворачиваются по правилам браузера: относительно той страницы,
    где ссылка найдена, с учётом перенаправлений и тега <base href>. Так
    разбор переживает смену вёрстки: опознаётся форма ссылки, а не шаблон
    страницы.
...
<#
    Сохранить страницу, на которой след оборвался.

    Без этого разбор чужой вёрстки превращается в переписку: «пришлите
    страницу» — «какую?» — и круг занимает день. Сохраняем последнюю
    осмотренную страницу, а не первую: именно на ней и кончается след. Рядом
    кладём её адрес, иначе непонятно, откуда она взялась.

    Первых пяти хватает: если вёрстка изменилась, она изменилась у всех.
#>
$script:DeadEnds = 0
function Save-DeadEnd {
    param([string]$Id, [string]$Html, [string]$Address)
    if ($script:DeadEnds -ge 5) { return }
    $script:DeadEnds++
    $склад = Join-Path $root 'не-найдено-ссылок'
    New-Item -ItemType Directory -Path $склад -Force | Out-Null
    $имя = Join-Path $склад ((Get-ItuFileName $Id) -replace '\.pdf$', '.html')
    $шапка = "<!-- $Id -->`n<!-- адрес: $Address -->`n"
    [IO.File]::WriteAllText($имя, ($шапка + $Html), (New-Object System.Text.UTF8Encoding($false)))
    if ($script:DeadEnds -eq 1) { Note "страница сохранена для разбора: $склад" }
}
...
foreach ($item in ($plan | Sort-Object series, number)) {
    ...
    $ключиИзданий = @()
    # Что показать человеку, если файл так и не найдётся: последняя из
    # осмотренных страниц, а не первая. Именно на ней и обрывается след.
    $последняяСтраница = $page
    $адресПоследней = $pageUrl

    # Вторая ступень. Ссылка из перечня ведёт на СПИСОК ИЗДАНИЙ
    # («recommendation.asp?parent=T-REC-A.31»), а файл лежит на странице
    # конкретного издания («…parent=T-REC-A.31-200810-I» — тот же
    # recommendation.asp, но с ключом издания). Берём свежие первыми: в отчёт
    # годится действующая редакция. Двух хватает: если и в предыдущей файла
    # нет, дело не в редакции.
    if (-not $ссылкаНаФайл) {
        $издания = @(Read-ItuEditionLinks -Html $page -Id $item.id) | Select-Object -First 2
        $ключиИзданий = @($издания | ForEach-Object { $_.id })
        foreach ($издание in $издания) {
            ...
            if ($DelayMs -gt 0) { Start-Sleep -Milliseconds $DelayMs }
            if (-not $страницаИздания) { continue }
            $последняяСтраница = $страницаИздания
            $адресПоследней = $script:LastUrl
            $найденная = Read-ItuPdfLink -Html $страницаИздания -Languages $Languages
            if ($найденная) {
                $ссылкаНаФайл = $найденная
                $pageUrl = $script:LastUrl
    ...
    <#
        Куда стучаться за файлом. Сначала то, что нашлось на страницах, —
        это главный путь. Если не нашлось ничего, остаётся собрать адрес из
        ключа издания: у МСЭ файл всегда отдаётся через dologin_pub.asp, а
        ключ издания мы уже знаем со страницы, не выдумываем.

        Крайняя мера, но безопасная: скачанное всё равно проверяется на
        подпись формата, поэтому промах не может положить в библиотеку
        что-нибудь постороннее — он просто ничего не даст. Зато там, где
        вёрстка страницы издания непривычна, документ всё-таки доедет.
        Языки перебираются по тому же порядку предпочтения.
    #>
    $кандидаты = @()
    $былаСсылка = [bool]$ссылкаНаФайл
    if ($былаСсылка) {
        $кандидаты += (Resolve-ItuUrl -Href $ссылкаНаФайл -PageUrl $основаСтраницы)
    } elseif ($ключиИзданий.Count) {
        foreach ($язык in $Languages) {
            $собранная = 'dologin_pub.asp?lang={0}&id={1}!!PDF-{2}&type=items' -f `
                         $язык.ToLower(), $ключиИзданий[0], $язык.ToUpper()
            $кандидаты += (Resolve-ItuUrl -Href $собранная -PageUrl $основаСтраницы)
        }
    }
    $кандидаты = @($кандидаты | Where-Object { $_ } | Select-Object -Unique)

    if (-not $кандидаты.Count) {
        Save-DeadEnd -Id $item.id -Html $последняяСтраница -Address $адресПоследней
        $nolink++
        $неудачи += [pscustomobject]@{ id = $item.id; этап = 'ссылка'; причина = 'на странице нет ссылки на PDF'; адрес = $адресПоследней }
    ...
    $причина = ''
    $последнийАдрес = ''
    for ($к = 0; $к -lt $кандидаты.Count; $к++) {
        $pdfUrl = $кандидаты[$к]
        $последнийАдрес = $pdfUrl
        $ответ = Invoke-Polite -Url $pdfUrl -Referer $адресПоследней -OutFile $target -Timeout 180
        $size = if (Test-Path -LiteralPath $target) { (Get-Item -LiteralPath $target).Length } else { 0 }
        $этоPdf = Test-PdfFile $target
        # Обрыв посреди передачи: сервер ответил 200, подпись %PDF- на месте,
        # а хвоста файла нет. Виден такой обрыв только по коду выхода curl.
        $оборван = ($ответ.exit -ne 0)
        if ($ответ.code -eq 200 -and $size -gt 4096 -and $этоPdf -and -not $оборван) {
            $взят = $true
            break
        }
        ...
        if ($DelayMs -gt 0 -and $к -lt ($кандидаты.Count - 1)) { Start-Sleep -Milliseconds $DelayMs }
    }
    ...
    if ($взят) {
        ...
        $broken++
        $неудачи += [pscustomobject]@{ id = $item.id; этап = 'файл'; причина = $причина; адрес = $последнийАдрес }
        if ($broken -le 10) { Warn "$($item.id) — $причина" }
        if ($broken -eq 11) { Note 'дальше об ошибках связи молчу, итог будет в конце' }
    } else {
        # Ссылки на странице не было, и собранный адрес тоже ничего не дал.
        # Скорее всего у этой рекомендации публикуется только платная версия.
        Save-DeadEnd -Id $item.id -Html $последняяСтраница -Address $адресПоследней
        $nolink++
        $неудачи += [pscustomobject]@{
            id = $item.id; этап = 'ссылка'
            причина = "файла нет ни на странице, ни по собранному адресу ($причина)"
            адрес = $адресПоследней
        }
        if ($nolink -le 10) { Warn "$($item.id) — на странице нет ссылки на PDF" }
        if ($nolink -eq 11) { Note 'дальше о таких молчу, итог будет в конце' }
    }
