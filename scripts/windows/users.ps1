<#
.SYNOPSIS
    Военнослужащие: список, создание, смена пароля.
.DESCRIPTION
...
.PARAMETER Add
    Логин нового военнослужащего. Пароль будет запрошен (не короче 8 символов).
.PARAMETER Reset
    Логин военнослужащего, которому нужно сменить пароль. Годится и для себя,
...
.PARAMETER Department
    Подразделение, в котором военнослужащий стоит ПО ШТАТУ, например "в/ч 74326".
...
if ($Add) {
    Write-Step "Создание военнослужащего $Add"
    ...
    if ($LASTEXITCODE -ne 0) { Write-Bad 'не удалось создать военнослужащего'; exit 1 }
