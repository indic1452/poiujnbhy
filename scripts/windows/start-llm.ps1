...
.PARAMETER Context
    Общий размер контекста в токенах на ВСЕ слоты. При -Parallel 1 весь он
    достаётся разговору целиком: 32768 токенов.

    Меньше нельзя: помощник кладёт в промпт до 52 000 знаков найденного
    материала и оставляет 4000 токенов на ответ. Если окно переполнить,
    llama.cpp молча выбрасывает начало промпта вместе с системной
    инструкцией — модель перестаёт ставить ссылки на источники и начинает
    отвечать по памяти. Со стороны выглядит как «модель поглупела», а
    причина в переполненном окне.
    ...
    в settings.json (52000 → 38000 → 23000 соответственно).
.PARAMETER Parallel
    Сколько разговоров модель ведёт одновременно. Стоит 1, и это осознанно:
    весь контекст достаётся одному разговору, ответы получаются полными.

    Цена — очередь. Второй спросивший ждёт, пока ответят первому; ждёт он
    молча внутри llama-server. При -Parallel 2 отвечают двоим сразу, но
    каждому достаётся половина окна (16384), и материала до модели доходит
    вдвое меньше — ответы становятся заметно беднее. Меняя это число,
    меняйте и assistant_context_chars в settings.json: 1 слот — 52000,
    2 слота — 23500.
.PARAMETER CpuMoe
