...
Остальные значения в образце уже подобраны под это железо: контекст, таймаут
600 с, две параллельные секции под `--parallel 2`, лимит загрузки 200 МБ.
...
Роли и их права — [docs/09 р. 9.2](09-web-app.md). Инженерам заводите
`--role engineer`, читателям `--role viewer`.
...
Поиск по этой библиотеке живёт в приложении (вкладка «Библиотека» и
`GET /api/search`). Команда `reportgen search` — про другое: она работает с
файловым индексом, который строит `reportgen index --corpus … --out …`, и нужна
для отладки конвейера без базы.
...
{"status": "ok", "database": "ok",
 "counts": {"users": 1, "documents": 128, "chunks": 4310, "cases": 0, "reports": 0, "edit_pairs": 0, "audit": 3},
...
перезапуск не нужен, каталог перечитывается ([docs/09 р. 9.6](09-web-app.md)).
...
| **Полный отчёт на 25–30 страниц (10–14 секций)** | **3–6 мин** на модели №1, **12–25 мин** на №2 | генерация идёт в HTTP-запросе ([docs/09 р. 9.8](09-web-app.md)) |
...
| 6 | Есть администратор | `sudo reportgen-cli users` | строка с ролью `admin` |
| 7 | Библиотека проиндексирована | `sudo reportgen-cli library` | документы и чанки, не «библиотека пуста» |
| 8 | Поиск находит | вкладка «Библиотека» в интерфейсе, запрос «занимаемая полоса» (или `GET /api/search?q=…` с сессионной cookie) | осмысленные фрагменты со ссылками на документы |
| 9 | Вход через браузер | `https://<сервер>/` | форма входа, сертификат принят |
| 10 | Сквозной прогон | создать кейс из `examples/cases/case-2024-118.json`, сгенерировать, утвердить, выгрузить DOCX | отчёт без ошибок верификатора |
...
| 15 | Журнал действий пишется | вкладка «Журнал» под администратором | видны входы и генерации |
...
2. уменьшите контекст: `LLAMA_CTX=8192` (посекционной генерации хватает);
