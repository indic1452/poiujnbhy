...
Документ описывает установку с нуля на сервер отдела: **RTX 4080 16 ГБ,
...
«Военнослужащие» или той же командой; должности: `head` — начальник отдела,
...
Группа (`--team`) заполняется не для порядка: по ней сводка собирает
списочный состав и нагрузку. Отдел у всех один — тот, что стоит в настройках
(`brand_name`); у каждого военнослужащего его не спрашивают. `--department` нужен
только тем, кто по штату числится в другом подразделении: в интерфейсе поле
подписано «По штату».
...
{"status": "ok", "database": "ok",
 "counts": {"users": 1, "documents": 128, "chunks": 4310, "cases": 0,
            "case_files": 0, "case_notes": 0, "reports": 0,
            "edit_pairs": 0, "chats": 0, "chat_messages": 0, "chat_attachments": 0,
            "absences": 0, "person_files": 0, "notifications": 0,
            "talks": 0, "talk_messages": 0, "audit": 3},
...
| 9 | Вход через браузер | `https://<сервер>/` | форма входа, сертификат принят, после входа открывается «Сводка» |
