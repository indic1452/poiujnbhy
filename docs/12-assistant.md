...
| | Отчёт | Помощник |
|---|---|---|
| Откуда числа | **Только из факт-пакета.** Всё остальное — ошибка, утверждение блокируется | Только из найденных фрагментов, с ссылкой [S<N>] |
...
Разговор принадлежит одному пользователю. Через интерфейс и через API чужой
чат недоступен **никому, включая администратора**: в вопросах инженеров
всплывают входящие номера писем, названия объектов и параметры линий
объектов — то же, что и в самих письмах (док. 07).

Полной тайны от администратора здесь нет и быть не может: он вправе сменить
военнослужащему пароль и войти под ним — так устроена любая система, где пароль
восстанавливают. Скрытно это не сделать: смена пароля пишется в журнал
(`user.password` с именем администратора), а у самого военнослужащего все сессии
закрываются, и в следующий раз он увидит окно входа. Если разговоры должны
быть закрыты и от администратора, это решается не программой, а порядком
доступа к машине.
...
## 12.2a. Разбор в несколько заходов
...
другими словами, чем написано в книге.

Теперь между вопросом и ответом идут короткие заходы. В каждом модель делает
один шаг и видит, что из него вышло:

| Шаг | Что происходит |
|---|---|
| `ИСКАТЬ: <запрос>` | ещё один поиск по библиотеке — своими словами, синонимом, английским термином, номером пункта |
| `ОГЛАВЛЕНИЕ: <документ>` | что вообще есть в этом томе |
| `ЧИТАТЬ: <документ> \| <раздел>` | прочитать названную главу целиком |
| `ХВАТИТ` | материала достаточно — или по этой теме в библиотеке явно ничего нет |

Число заходов — `assistant_rounds` (по умолчанию 4). Ноль возвращает прежнее
поведение: один поиск, один проход. Каждый заход стоит короткого обращения к
модели (ответ в одну строку), поэтому на медленной машине число имеет смысл
снизить.

Находки всех заходов сводятся в один список слиянием по обратным рангам
(RRF): шкалы BM25, косинуса и реранка между собой несравнимы, а места в
списках — сравнимы. Фрагмент, всплывший в двух заходах по разным словам,
поднимается выше — это именно то, что нужно: два независимых способа его
найти. Прочитанное по шагу `ЧИТАТЬ` ставится первым и вытеснению не подлежит:
его запросили по имени.

**Протокол текстовый, а не «вызов функции».** Модель здесь локальная и
меняется от установки к установке. Строгий JSON такие модели держат неровно и
молча ломаются на первом лишнем слове, а «ИСКАТЬ: кадр BBFrame» разбирается
как угодно снисходительно — по-русски и по-английски, с двоеточием и без,
после рассуждения вслух. Тем же способом в системе работает реранк моделью.

**Непонятый шаг — не ошибка, а конец разбора.** Помощник отвечает по тому,
что успел собрать. Переспрашивать бессмысленно: модель, не сумевшая написать
строку по образцу, со второй попытки её обычно тоже не пишет, а инженер всё
это время ждёт. Отказ модели на заходе, пустая библиотека и `assistant_rounds
= 0` ведут туда же — к ответу по первому поиску.

**Ход разбора виден инженеру.** В панели источников появляется список: что
искали, какое оглавление смотрели, что читали и чем каждый заход кончился
(«ничего нового не нашлось»). Без него ответ «в библиотеке этого нет»
невозможно ни проверить, ни оспорить.

## 12.3. Как собирается материал для модели

0. **Карта библиотеки** — что вообще лежит на полках: направление, сколько
   в нём документов и фрагментов, названия самых объёмных. Полки, которых
   коснулся поиск (и направление разговора, если оно задано), называются
   первыми. Без карты помощник знал о библиотеке только то, что попало в
   найденные фрагменты, и вёл себя одинаково плохо в двух случаях: когда
   нужное лежит в соседнем томе, до которого поиск не дотянулся, и когда по
   теме в библиотеке нет ничего — сказать об этом ему было неоткуда, и он
   рассуждал по памяти так же уверенно, как по документу. Карта говорит
   модели, ГДЕ искать; числа и утверждения по-прежнему берутся только из
   фрагментов со ссылками. Объём — `assistant_catalog_chars` (2500 знаков);
   ноль убирает карту совсем. Заменённые (`superseded`) документы в карту не
   попадают: направлять к ним инженера незачем.
...
4. **Соседние куски.** К первым `assistant_neighbour_top` фрагментам (4)
   ...
   текст занимал бы окно дважды. Предыдущий сосед обрезается СПЕРЕДИ, а не
   с конца: к найденному фрагменту примыкает его хвост, ради этого стыка
   соседа и берут. В промпте соседи помечены как «предыдущий /
...
В эти 26 000 знаков входит всё, что зависит от вопроса: приложенные файлы,
разговор, сам вопрос, карточка письма, карта библиотеки, оглавления найденных
документов и сами фрагменты. Граница держится сверху вниз, в порядке
убывающей ценности:

1. **Карта библиотеки** ужимается первой — от неё остаётся перечень полок
   без названий документов, а на совсем тесном окне она снимается вовсе.
2. **Разговор** теряет самые старые реплики; последняя пара остаётся всегда.
3. **Приложенные файлы** обрезаются по остатку окна, даже если
   `assistant_attachment_chars` разрешает больше. Обрезка видна: в шапке
   файла стоит «показано N из M знаков».
4. **Оглавления** найденных документов.
5. **Хвост подборки** — фрагменты с самой низкой относимостью.

Фрагментам гарантирован пол: без источников помощник превращается в обычную
модель без ссылок на нормы. Раньше этот пол был односторонним — источники в
него упирались, а промпт всё равно вылезал за окно: при вложении в 40 000
знаков он вырастал до 47 678 при границе 26 000, и llama.cpp молча выбрасывал
его начало вместе с системной инструкцией. Со стороны это выглядело как
«модель поглупела»: пропадали ссылки [S1], появлялись округлённые числа.
...
- **В промпт идёт начало файла** — `assistant_attachment_chars` знаков (8000)
  на все приложенные файлы вместе. Дамп на десятки мегабайт в окно не
  поместится никогда, а в начале лежат заголовки сессии и первые ошибки, по
  которым обычно и понятно, что случилось. Об обрезке модели сказано прямо —
  иначе она сделает вывод «ошибок больше нет» по обрезанному хвосту.
  Предел делится между файлами поровну, но короткая записка не занимает
  чужого: то, что она не выбрала, достаётся дампу. Раньше предел был на
  каждый файл — десять приложенных файлов выносили промпт за окно модели
  втрое, и llama.cpp молча выбрасывал начало промпта вместе с системной
  инструкцией: модель переставала ставить ссылки.
...
номер группы, оборудование, суть обращения, полученные материалы) и таблица
...
- Не заменяет отчёт и сам документом не является.
...
- **`найдено фрагментов: N в M док., процитировано: 0`** — фрагменты нашлись,
  ...
  пользовалась модель.
- **`найдено фрагментов: N (модель видела K)`** — часть найденного не влезла
  в окно модели. Само по себе это не беда (хвост выдачи наименее относим), но
  если K сильно меньше N, стоит сузить вопрос: ответ построен не на всём, что
  нашлось. Убрать оговорку можно, увеличив `assistant_context_chars` вместе с
  контекстом llama-server (см. 12.4).
- **Ссылка `[S<N>]` перечёркнута и не нажимается** — модель сослалась на
  фрагмент, которого в подборке нет. Это выдумка: утверждение рядом с такой
  ссылкой ничем не подтверждено. В счётчик «процитировано» такие ссылки не
  идут.
...
Учётная запись (логин, ФИО, должность, группа, перечень прав), сводка
...
Смена пароля закрывает все прочие сессии этого военнослужащего, а текущая выдаётся
...
| `assistant_attachment_chars` | Сколько знаков всех приложенных файлов вместе уходит в промпт (8000) |
...
| `assistant_catalog_chars` | Сколько знаков занимает карта библиотеки (0 — не показывать) |
| `assistant_rounds` | Сколько заходов разбора делать до ответа (0 — один поиск, как раньше) |
