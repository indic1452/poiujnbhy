...
номеру, теме и номеру группы. В таблице: входящий номер, тема, номер группы,
...
Три перехода система делает сама: генерация переводит письмо в «в работе»,
отправка отчёта на проверку — в «на проверке», отметка «проверен» — в
«отправлено». «В архиве» ставит человек в карточке — система не знает, закрыт
ли вопрос.

Два из этих состояний — «на проверке» и «отправлено» — руками не выставляются,
пока по письму есть хотя бы один отчёт: их даёт проверка отчёта, и в карточке
они заперты. Иначе исполнитель ставил письму «отправлено», ничего не сдав:
письмо уходило из работы, начальник его больше не видел, а отчёт никто не
проверял. Письмо, по которому нет ни одного отчёта, — другое дело: на него
ответили мимо системы, подменять нечего, и отметить это можно.
...
так далее, старые остаются доступны. Номер версии присваивается под замком
