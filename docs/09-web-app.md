...
полную яркость. При нажатии пеленг взят — развёртка встаёт, луч зеленеет,
эмблема слегка вдавливается; отпустили — ход продолжается с той же точки, без
скачка. Щелчок ведёт на дашборд: знак организации всегда ведёт домой.
...
Стоит это дёшево: один `transform` на группе из шести путей, остальное —
переходы прозрачности. Ни фильтров, ни размытий, ни градиентов, ни единой
