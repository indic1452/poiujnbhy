...
#:            администраторов и безвозвратное удаление военнослужащих;
...
ROLE_NOTES = {
    "owner": "Полные права: проверяет отчёты, ведёт военнослужащих, библиотеку, "
    ...
    "head": "Проверяет отчёты отдела: отмечает проверенными или возвращает "
            "с замечанием. Ведёт письма, заводит и отключает военнослужащих, "
            "видит нагрузку всего отдела.",
    "deputy": "То же, что начальник отдела, включая проверку отчётов.",
    "lead": "Ведёт письма и готовит отчёты, заводит и отключает военнослужащих "
...
#: Документы военнослужащего. Главный из них — справка-объективка, её в отделе
...
class User:
    ...
    #: отделе — это и есть система, — а по штату военнослужащий может числиться в
    ...
    def is_admin(self) -> bool:
        """Заводит и отключает военнослужащих, удаляет документы, читает журнал."""
...
@dataclass
class PersonFile:
    """Документ военнослужащего: справка-объективка, приказ, прочее."""
...
@dataclass
class Absence:
    """Период отсутствия или дежурства военнослужащего."""
