...
class Settings:
    ...
    #
    # АРИФМЕТИКА ОКНА. llama-server запускается с -c 32768 --parallel 1, то
    # есть весь контекст достаётся одному разговору: 32768 токенов. Из них:
    #   assistant_max_tokens          — ответ                     4000
    #   системная инструкция          — ~1800 (см. ниже)
    #   история разговора             — ~600
    #   запас на токенизацию          — ~4700
    #   остаётся под материал         — примерно 21 700 токенов
    # Русский текст при этой токенизации — около 2,4 знака на токен, значит
    # материал не должен превышать ~52 000 знаков. Это и есть
    # assistant_context_chars: жёсткая граница, ниже которой блок ИСТОЧНИКИ
    # обрезается по одному фрагменту с конца выдачи.
    #
    # Число слотов и эта настройка связаны жёстко. Поставите --parallel 2 —
    # каждому разговору достанется половина окна, и материал надо уменьшить
    # до 23 500, иначе начало промпта вместе с инструкцией уедет молча.
    #
    # Оценку инструкции держите в согласии с prompts.py: она в бюджет окна не
    # входит расчётом (assistant.py считает только вопрос, карточку, историю и
    # материал), поэтому ошибка в ней ничем не ловится, кроме этой строки.
    # Правило 5 о разметке добавило ей около 900 токенов — на столько же
    # уменьшен материал. Сверить размер: len(ASSISTANT_SYSTEM_PROMPT) / 2,4.
    #
    ...
    assistant_context_chars: int = 52000
    ...
    #: Восемнадцать — под окно в 32768 токенов (один слот). При двух слотах
    #: половина из них всё равно не влезет и будет отброшена: ставьте 12.
    assistant_top_k: int = 18
    ...
    host: str = "127.0.0.1"
    port: int = 8080
    #: Работать по https со своим сертификатом.
    #:
    #: Нужно ровно для одного: уведомление на рабочем столе браузер
    #: показывает только «защищённой» странице — https или адрес самой
    #: машины с сервером. По http на адрес в сети такого окна у браузера нет
    #: вовсе: человек свернул окно, его вызывают в кабинет, а он не знает.
    #:
    #: Сертификат система выписывает себе сама при первом запуске (купить
    #: его в изолированном контуре не у кого). Браузер при первом заходе
    #: скажет, что не знает такого удостоверителя, — это лечится один раз на
    #: каждой машине, см. docs/11-windows.md.
    https: bool = False
    #: Дополнительные имена и адреса в сертификате. Система сама вписывает
    #: имя машины и её адреса, но если к ней обращаются по чему-то ещё
    #: (второй сетевой адрес, псевдоним в hosts), допишите это сюда —
    #: браузер сверяет адрес в строке с тем, что в сертификате, и на
    #: несовпадение ругается отдельно.
    https_hosts: list = field(default_factory=list)
