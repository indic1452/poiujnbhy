...
class Settings:
    ...
    # Три величины, от которых зависит, насколько ответ полон. Их держат
    # вместе, потому что вместе они и упираются в окно контекста модели
    # (по умолчанию 16384 токена, ключ -c у llama-server).
    #
    #: Сколько знаков каждого найденного фрагмента видит модель. Это главный
    #: рычаг: короткий фрагмент обрывается на середине таблицы допусков, и
    #: писать модели просто не из чего. Считать так: retrieval_top_k ×
    #: assistant_source_chars ≈ объём материала; 8 × 1400 ≈ 11 тыс. знаков,
    #: это примерно 4 тыс. токенов.
    assistant_source_chars: int = 1400
