...
class Settings:
    ...
    #: Сколько знаков приложенного файла (дампа, лога, документа) видит
    #: модель. Дамп на 40 МБ в окно не влезет никогда, поэтому берётся
    #: начало — там заголовки сессии и первые ошибки, по которым обычно и
    #: понятно, что случилось. Входит в общий бюджет assistant_context_chars.
