...
def separate_code_params(text: str) -> str:
    """Запятые в скобочной записи кодов заменить пробелами.

    «RS (204,188,12)» — три параметра кода, а не дробь: длина кодового слова,
    число информационных символов и исправляющая способность. Тому, кто
    разбирает текст на слова, это надо знать отдельно — иначе десятичная
    запятая и запятая-разделитель выглядят одинаково. «(13,7 дБ)» под правило
    не подпадает и остаётся дробью.
    """
    if "," not in text:
        return text
    return _CODE_PARAMS_RE.sub(lambda m: m.group(0).replace(",", " "), text)
