...
#
# Первым идёт число с десятичной ЗАПЯТОЙ — так отдел пишет все свои величины.
# Без него «12,5 дБ» распадалось на «12» и «5», и запрос про порог 12,5 дБ
# одинаково хорошо оценивал фрагмент про 1,5 дБ: общий у них токен «5». Запятая
# в скобочной записи кодов («RS (204,188,12)») разделяет параметры, а не
# отделяет дробную часть, поэтому цепочка из двух запятых под правило не
# подпадает — там по-прежнему три отдельных числа.
_TOKEN_RE = re.compile(
    r"(?<![,\d])\d+,\d+(?![,.]?\d)"
    r"|[a-zA-Zа-яА-ЯёЁ0-9]+(?:\.[0-9]+)+"
    r"|[a-zA-Zа-яА-ЯёЁ0-9]+"
)

#: Буквы, неотличимые на глаз в кириллице и латинице. Тот же набор пар, что и
#: в :mod:`reportgen.ingest.text_repair` (там он читается в другую сторону);
#: согласованность двух таблиц заперта тестом.
#:
#: Зачем это поиску. Обозначения отдел печатает в двух раскладках вперемешку:
#: «Е1» с русской Е и «E1» с латинской — на экране одно и то же слово, а для
#: указателя два разных, и запрос находит ровно половину библиотеки. То же
#: делает распознавание сканов: на замере «E1» превращалось в «Е1» кириллицей.
#: Приводим такие слова к одному виду — и в указателе, и в запросе.
_CONFUSABLE = {
...
def unmix_scripts(token: str) -> str:
    """Слово-обозначение — к одной раскладке.

    Трогаем только то, что и так двусмысленно: слово, где перемешаны кириллица
    и латиница, либо слово из букв и цифр («Е1», «ФМ4», «G.703»). Обычное
    русское слово и обычное английское остаются как есть — сводить их не к
    чему, а испортить можно.
    """
    if not (_CYRILLIC.search(token) and (_LATIN.search(token) or _DIGIT.search(token))):
        return token
    return "".join(_CONFUSABLE.get(letter, letter) for letter in token)
